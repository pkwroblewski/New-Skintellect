...
import { Bookmark, ShoppingBag, X, ArrowLeft } from 'lucide-react';
...
export default function SavedItemsPage() {
  ...
  const { savedForLater, toggleSaveForLater, addToCart } = useCartStore();
  ...
  const handleRemove = (productId: string, productName: string) => {
    ...
    toast.info(`${productName} removed from saved items`);
  };
    ...
    return (
      <section className="min-h-[70vh] flex items-center justify-center bg-[#FAF7F2]">
        <div className="text-center">
          ...
          <p className="text-slate-400 font-serif italic">Loading saved items...</p>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="min-h-[70vh] flex items-center justify-center bg-[#FAF7F2]">
        <div className="text-center max-w-md px-6">
          <div className="w-24 h-24 mx-auto mb-8 rounded-full bg-indigo-50 flex items-center justify-center">
            <Bookmark size={40} className="text-indigo-400" />
          </div>
          <h1 className="text-4xl font-serif text-slate-900 mb-4 italic">No saved items</h1>
          <p className="text-slate-400 font-serif text-lg mb-8">
            Save products for later by clicking the bookmark icon.
          </p>
          <Link
            href="/"
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-12">
        ...
        <div className="flex items-center justify-between mb-16">
          <div>
            ...
            <h1 className="text-5xl font-serif text-slate-900 italic">Saved for Later</h1>
            <p className="text-slate-400 mt-2">{savedProducts.length} item{savedProducts.length !== 1 ? 's' : ''}</p>
          </div>
          <button
